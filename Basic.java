...
public class Basic{
  ...
  public final static int retinaFactor = 1;  // 2 for Retina Display, else 1
  ...
  // create window given title, pixels to shift window to right, size in pixels, step time in nanoseconds
  public Basic( String windowLabel, int ws, int pw, int ph, long timeInNanos )
  {
    title = windowLabel;
    width = pw;  height = ph;
...
System.out.println("framebuffer size: " + widthArray[0] + " " + heightArray[0] );
    ...
    glfwSetCursorPosCallback( window,
       cursorPosCallback = new GLFWCursorPosCallback()
       {
         ...
         {
           // whenever cursor moves, update mouse location and add input info
           mouseX = (int) Math.round(xpos);
           mouseY = (int) Math.round(ypos);
           InputInfo.add( new InputInfo( 'm', mouseX, mouseY ) );
         }
       }
     );
    ...
    glfwSetWindowPos( window, windowShift, 0 );

/*  this didn't work---trying to get actual pixel size,
    like for retina display half-size issuze
    int[] ww = new int[1];
    int[] hh = new int[1];
    glfwGetFrameBufferSize( window, ww, hh );
    System.out.println( "window size: " + ww[0] + " " + hh[0] );
*/
...
//    GLContext.createFromCurrent();
  ...
  {
    Basic t = new Basic( "Basic App (should be extended)", 10, 800, 600, 33333333L );
    t.start();
  }
